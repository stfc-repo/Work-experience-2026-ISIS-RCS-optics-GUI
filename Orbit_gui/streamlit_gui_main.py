...
def get_error_table_orbit_snapshot(cycle_time_ms, requested_qx, requested_qy, _corrector_settings=None):
    # _corrector_settings is prefixed with an underscore so Streamlit's cache
    # does not try to hash the SnapshotCorrectorSettings object itself --
    # only the primitive args above are used as the cache key.
    config = copy_snapshot_config(
        orbit_base_config,
        ...
        error_table_paths=[str(error_table_path)] if error_table_path.exists() else [],
        orbit_correction_configs=[],
        corrector_settings=_corrector_settings,
        run_envelope=False,
        run_aperture=False,
    )
    return build_machine_snapshot(config)
...
# ---------------------------------------------------------------------------
# Live data from the ISIS archiver (optics_gui.io.epics_live /
# epics_archiver_client). Each helper below builds the fetch_value /
# list_available_times callables the epics_live functions expect, then
# calls straight into them -- the same wiring as Dev/12_IO/archiver_live_app.py.
#
# Cached with a short ttl (rather than forever, like the MAD-X snapshots
# above) since live archiver values are expected to change over time,
# unlike a MAD-X run for a fixed cycle_time/tune/error-table combination.
# ---------------------------------------------------------------------------
@st.cache_data(ttl=30, show_spinner="Reading live tune from the archiver...")
def get_live_tune(cycle_time_ms, base_url):
    ...
    return get_requested_tune(cycle_time_ms, fetch_value, list_available_times_dwtrim)


@st.cache_resource(ttl=30, show_spinner="Reading live corrector currents...")
def get_live_corrector_settings(cycle_time_ms, base_url):
    def fetch_value(pv_name, as_of=None):
        return archiver_fetch_value(pv_name, as_of=as_of, base_url=base_url)
    ...
    return get_corrector_settings(cycle_time_ms, fetch_value, list_available_times)


@st.cache_data(ttl=30, show_spinner="Reading live BPM measurements...")
def get_live_bpm_measurements(cycle_time_ms, requested_qx, requested_qy, base_url, pv_pattern):
    # BPM positions (s) are static lattice geometry -- take them from the
    # already-built nominal snapshot rather than querying EPICS for them.
    snapshot = get_nominal_orbit_snapshot(cycle_time_ms, requested_qx, requested_qy)
    geometry = bpm_geometry_table(snapshot.table("twiss"))

    def fetch_value(pv_name, as_of=None):
        return archiver_fetch_value(pv_name, as_of=as_of, base_url=base_url)

    def pv_name_for_bpm(bpm_label, plane):
        return pv_pattern.format(bpm=str(bpm_label).upper(), plane=plane)

    return get_bpm_measurements(geometry, fetch_value, pv_name_for_bpm)
...
with st.sidebar:
    ...
    st.header("Live data source")
    data_source = st.radio(
        "Tune / correctors / BPMs from",
        ["Manual", "Live (ISIS archiver)"],
        index=0,
        key = 'data_source_radio',
        help="Live reads real values from the ISIS archiver at the cycle time above. "
        "Requires network access to the archiver, so it will only work when this app "
        "is actually run on the ISIS network -- not from a hosted/cloud deployment.",
    )
    use_live_data = data_source == "Live (ISIS archiver)"

    archiver_base_url = DEFAULT_BASE_URL
    bpm_pv_pattern = "UNCONFIRMED::{bpm}:{plane}"
    if use_live_data:
        archiver_error = False
        try:
            live_tune_row, live_tune_missing = get_live_tune(cycle_time_ms, archiver_base_url)
        except Exception as exc:
            st.session_state["archiver_warning"] = (
                f"Could not reach the archiver! Please consult the following error:  \n  \n({exc})"
            )
            live_tune_row, live_tune_missing = None, None
            archiver_error = True

        if archiver_error:
            # Connection failed outright -- use silent defaults, no widgets shown.
            requested_qx = 4.31
            requested_qy = 3.83
        elif live_tune_row is None:
            requested_qx = st.number_input("Set Qx (horizontal tune)", value=4.31, step=0.01, format="%.3f")
            requested_qy = st.number_input("Set Qy (vertical tune)", value=3.83, step=0.01, format="%.3f")
        else:
            requested_qx = live_tune_row["set_qx"]
            requested_qy = live_tune_row["set_qy"]
            col1, col2 = st.columns(2)
            col1.metric("Live Qx (H_Q)", f"{requested_qx:.4f}")
            col2.metric("Live Qy (V_Q)", f"{requested_qy:.4f}")
            if live_tune_missing:
                st.session_state["archiver_warning"] = f"Missing: {live_tune_missing}"
    else:
...
if st.session_state.get("archiver_warning"):
    st.warning(st.session_state.pop("archiver_warning"))
    if st.button("Switch back to manual"):
        st.session_state["data_source_radio"] = "Manual"
        st.rerun()
live_corrector_settings = None
if use_live_data and not archiver_error:
    try:
        live_corrector_settings, live_corrector_missing = get_live_corrector_settings(
            cycle_time_ms, archiver_base_url
        )
        if live_corrector_settings is None:
            st.warning("No live corrector data available at this cycle time -- using the error table alone.")
        elif live_corrector_missing:
            st.warning(f"No live data for {len(live_corrector_missing)} corrector(s): {live_corrector_missing}")
    except Exception as exc:
        st.error(f"Failed to fetch live corrector settings: {exc}")

try:
    nominal_snapshot = get_nominal_orbit_snapshot(cycle_time_ms, requested_qx, requested_qy)
    error_snapshot = get_error_table_orbit_snapshot(
        cycle_time_ms, requested_qx, requested_qy, _corrector_settings=live_corrector_settings
    )
except Exception as exc:
    st.error(
        f"MAD-X could not match the requested tunes (Qx={requested_qx:.3f}, Qy={requested_qy:.3f}) "
        f"at cycle time {cycle_time_ms:.1f} ms. This usually means the tune is too close to a "
        f"resonance or outside the achievable range at this point in the cycle.\n\n"
        f"Error detail: {exc}"
    )
    st.stop()
...
if orbit_mode == "Nominal":
    ...
    if use_live_data and not archiver_error:
        st.subheader("Measured BPM orbit (live)")
        st.caption(
            "BPM PV names are not yet confirmed by ISIS controls staff -- see the pattern "
            "entered in the sidebar and epics_live.py's module note. Treat these readings "
            "as provisional until that mapping is confirmed."
        )
        try:
            measured_bpm = get_live_bpm_measurements(
                cycle_time_ms, requested_qx, requested_qy, archiver_base_url, bpm_pv_pattern
            )
            measured_bpm = measured_bpm[measured_bpm["plane"] == bpm_plane].copy()
        except Exception as exc:
            st.error(f"Failed to fetch live BPM measurements: {exc}")
            st.info("Falling back to the dummy example orbit below.")
